...
typedef enum  { NUL, IDENT, NUMBER, PLUS, MINUS, TIMES,
	            ...
	            CONSTSYM, VARSYM, PROCSYM, PROGSYM, FORSYM,
                    ELSESYM, STEPSYM, UNTILSYM, TYPEDEFSYM,ANNOSYM
        } SYMBOL;
...
void GetSym() {
  ...
  if (CH>='A' && CH<='Z') { /*ID OR RESERVED WORD*/
	...
	if (i-1 > J) SYM=WSYM[K]; /*reserved word*/
	else SYM=IDENT; /*variable name*/
  }
  else
    if (CH>='0' && CH<='9') { /*NUMBER*/
	    ...
	    if (CH=='<') {
		  ...
		  if (CH=='=') { SYM=LEQ; GetCh(); }
		    ...
		    GetCh();
             ...
             if(CH=='*'){
                GetCh();
                if(CH=='='){SYM=MULP;GetCh();}
                else SYM = TIMES;
             }
      else
             if (CH=='/'){
                GetCh();
                if(CH=='='){SYM=DIVI;GetCh();}
                else if(CH=='/'){SYM=ANNOSYM;GetCh();}
                else SYM = SLASH;
             }
...
void TERM(SYMSET FSYS, int LEV, int &TX) {  /*TERM*/
  SYMBOL MULOP;
  if(SYM==MULP||SYM==DIVI){
  MULOP = SYM;GetSym();
  FACTOR(SymSetUnion(FSYS,SymSetNew(TIMES,SLASH,MULP,DIVI)),LEV,TX);
  if(MULOP==MULP) GEN(OPR,0,30);
  else GEN(OPR,0,31);
  }
  else FACTOR(SymSetUnion(FSYS,SymSetNew(TIMES,SLASH,MULP,DIVI)), LEV,TX);
  while (SYM==TIMES || SYM==SLASH||SYM==MULP||SYM==DIVI) {
	MULOP=SYM;  GetSym();
	FACTOR(SymSetUnion(FSYS,SymSetNew(TIMES,SLASH,MULP,DIVI)),LEV,TX);
	if (MULOP==TIMES) GEN(OPR,0,4);
	else if(MULOP==SLASH) GEN(OPR,0,5);
        else if(MULOP==MULP) GEN(OPR,0,30);
        else  GEN(OPR,0,31);
  }
} /*TERM*/;
//---------------------------------------------------------------------------
...
void CONDITION(SYMSET FSYS,int LEV,int &TX) {
  ...
  if (SYM==ODDSYM) { GetSym(); EXPRESSION(FSYS,LEV,TX); GEN(OPR,0,6); }
	...
	EXPRESSION(SymSetUnion(SymSetNew(EQL,NEQ,LSS,LEQ,GTR,GEQ,MULP,DIVI),FSYS),LEV,TX);
	if (!SymIn(SYM,SymSetNew(EQL,NEQ,LSS,LEQ,GTR,GEQ,MULP,DIVI))) Error(20);
	else {
	  ...
	  switch (RELOP) {
	    ...
	    case LEQ: GEN(OPR,0,13); break;
            case MULP: GEN(OPR,0,30); break;
            case DIVI: GEN(OPR,0,31); break;
	  }
	}
  }
} /*CONDITION*/
...
void STATEMENT(SYMSET FSYS,int LEV,int &TX) {   /*STATEMENT*/
  ...
  switch (SYM) {
	...
	case WHILESYM:
        ...
        case FORSYM:
                GetSym();
                Form1->printfs("for²âÊÔ");
                break;

/*        case ELSESYM:
                GetSym();
                Form1->printfs("else²âÊÔ");
                break;
...
*/
        ...
        case STEPSYM:
                GetSym();
                Form1->printfs("step²âÊÔ");
                break;
        case UNTILSYM:
                GetSym();
                Form1->printfs("until²âÊÔ");
                break;
        case TYPEDEFSYM:
                GetSym();
                Form1->printfs("typedef²âÊÔ");
                break;
        case ANNOSYM:
                GetSym();
                Form1->printfs("ÎÒÒ²²»ÖªµÀÔõÃ´×¢ÊÍ");
                break;
      ...
      case OPR:
	    switch (I.A) { /*OPERATOR*/
	      ...
	      case 13: T--; S[T]=S[T]<=S[T+1];  break;
              case 30: T--; S[T]=S[T]*S[T+1];   break;
              case 31: T--; S[T]=S[T]/S[T+1];   break;
...
void __fastcall TForm1::ButtonRunClick(TObject *Sender) {
  ...
  strcpy(KWORD[13],"STEP");
  strcpy(KWORD[14],"THEN");     strcpy(KWORD[15],"TYPEDEF");
  strcpy(KWORD[16],"UNTIL");    strcpy(KWORD[17],"VAR");
  ...
  WSYM[13]=STEPSYM;
  WSYM[14]=THENSYM;    WSYM[15]=TYPEDEFSYM;
  WSYM[16]=UNTILSYM;   WSYM[17]=VARSYM;
  ...
  if ((FIN=fopen((Form1->EditName->Text+".PL0").c_str(),"r"))!=0) {
	...
	if (SYM!=PROGSYM) Error(0);  /*program keyword*/
	else {
	  GetSym();
	  if (SYM!=IDENT) Error(0);    /*variable ->ID*/
	  else {
		GetSym();
